...
def pretty(cid: str) -> str:
    ...
    out = (
        out.replace("radius", "r").replace("degree", "d")
        .replace("iterations", "it").replace("kappa", "$kappa$")
    )
...
print(f"shards={len(fs)} snr={SNRS} seeds={df['seed'].n_unique()} configs={df['filter_config_id'].n_unique()}")
...
def bold_per_row(rows: list[list], p: int, dirn: str = "min") -> list[list]:
    ...
    for label, vals in rows:
        ...
        for i, v in enumerate(vals):
            s = f(v, p)
            if v is not None and target is not None and round((abs(v) if dirn == "zero" else v), p) == target:
                s = "*" + s + "*"
            cells.append(s)
        out.append(cells)
    return out
...
def cpgf_matrix(metric: str, fn: str, p: int, dirn: str):
    g = agg(cp, metric, fn).with_columns(
    ...
    for s in SNRS:
        ...
        for r in RADII_D1:
            ...
            vals.append(cell[metric][0] if cell.height else None)
...
for s in SNRS:
    ...
    rows.append([
        snr_label(s),
        f(sub["frac"].median() * 100, 3),
        f(sub["frac"].max() * 100, 3),
    ])
write(
...
recs = []  # (family, cid, [pretty, v@0, v@5, v@10, v@20, v@30])
for cid in sorted(df["filter_config_id"].unique().to_list(), key=natkey):
    ...
    for s in CAT_SNRS:
        ...
        vals.append(cell["nrmse"][0] if cell.height else None)
...
for fam in sorted({fam for fam, _, _ in recs}, key=fam_order):
    ...
    for ci_idx in range(1, 1 + len(CAT_SNRS)):
        vals = [(i, c[ci_idx]) for i, (_, c) in enumerate(grp) if isinstance(c[ci_idx], (int, float))]
        if not vals:
            continue
    ...
    for ri, (_, cells) in enumerate(grp):
        ...
        for ci_idx in range(1, 1 + len(CAT_SNRS)):
            v = cells[ci_idx]
            s = f(v, 2)
            if v is not None and ri in best.get(ci_idx, set()):
                s = "*" + s + "*"
            row.append(s)
        cat_rows.append(row)
write(
