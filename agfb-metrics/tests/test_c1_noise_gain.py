"""Tests for C.1 background noise gain."""
...
def test_half_normal_gain_matches_theory() -> None:
    ...
    std = sigma * sqrt(1 - 2/pi), so C.1 = sqrt(1 - 2/pi) ≈ 0.603."""
    torch.manual_seed(0)
    sigma_n = 0.7
    H = W = 256
    ...
    out = c1_noise_gain(gx, gy, mask, sigma_n=sigma_n)
...
def test_rayleigh_gain_matches_theory() -> None:
    ...
    std = sigma * sqrt(2 - pi/2), so C.1 = sqrt(2 - pi/2) ≈ 0.655."""
    torch.manual_seed(1)
    sigma_n = 1.3
    H = W = 256
    gx = sigma_n * torch.randn(1, H, W)
...
def test_zero_when_filter_is_zero() -> None:
    ...
    out = c1_noise_gain(gx, gy, mask, sigma_n=1.0)
...
def test_rejects_invalid_sigma() -> None:
    ...
    with pytest.raises(ValueError):
        c1_noise_gain(gx, gy, mask, sigma_n=0.0)
    with pytest.raises(ValueError):
        c1_noise_gain(gx, gy, mask, sigma_n=-1.0)
