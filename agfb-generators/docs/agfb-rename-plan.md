...
The generator component should be renamed from `cpgf-generators` to `agfb-generators`, with the Python package renamed from `cpgf_generators` to `agfb_generators`. This package owns analytic stimulus generation. Each generator produces a closed-form intensity field and an analytic ground-truth gradient field for filter evaluation.
...
The implementation order should start with `agfb-generators` because this repository already contains that component. After that, update dependent packages in order. Rename imports in the operators, metrics, band-limit experiment, pipeline, tests, and documentation only after the generator package provides the new import path.
