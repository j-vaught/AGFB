...
from agfb_generators import polynomial  # noqa: E402

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
with torch.no_grad():
    frame = polynomial(160, 160, device=device)
...
axes[1].imshow(gradient_angle, cmap=angle_cmap, vmin=-math.pi, vmax=math.pi)
...
plt.savefig(Path(__file__).with_name("polynomial_preview.png"), bbox_inches="tight", pad_inches=0)
